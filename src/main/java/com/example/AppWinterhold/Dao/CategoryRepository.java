...
public interface CategoryRepository extends JpaRepository<Category,String> {
    ...
    @Query("""
            SELECT new com.example.AppWinterhold.Dto.Category.CategoryIndexDto
            (
    ...
    @Query("""
            SELECT new com.example.AppWinterhold.Dto.Category.CategoryInsertDto
            (
                c.name, c.floor, c.isle, c.bay
            )
            FROM Category as c
    ...
    @Query("""
            SELECT COUNT(c.name)
            FROM Category AS c
            WHERE c.isle = :isle AND c.floor = :floor AND c.bay = :bay
            """)
